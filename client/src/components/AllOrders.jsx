...
const URL = "https://ecommserver-pado7k34.b4a.run/allOrders132"

const AllOrders = () => {

  const [orders, setOrder] = useState([]);

  const [user, setuserData] = useState({ userID: "" });
  ...
  const findOrder = (e) => {
    ...
    var newPageURL = 'https://client-kappa-rouge-53.vercel.app/allOrders/' + userID;
  ...
  return (
    <Wrapper>
      <div className="order-container">
        ...
        <form>
          <input type="text" name='userID' value={user.userID} onChange={handleInputs} placeholder='userID' />
          <br />
          <br />
          ...
          {orders.map((order, index) => (
            <li key={index} className="order-card">
              <div>
                ...
                <NavLink to={`/allOrders/${order.userID}`} className="order-link">View Details</NavLink>
              </div>
            </li>
          ))}
        </ul>
